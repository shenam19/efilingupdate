@foreach ($folders as $folder)
    <div class="file d-flex justify-content-between p-2 bg-default">
        @if ($folder->subfolders_count)
            <div class="dropdown-btn">
                <button class="btn p-0" data-toggle="collapse" href="#subfolder_{{ $folder->id }}" role="button"
                    aria-expanded="false" aria-controls="collapseExample">
                    <i class="fas fa-caret-right"></i>
                </button>
            </div>
        @endif
        <button class="btn p-0 file-info d-flex flex-grow-1"
            wire:click="$dispatch('setFile',{ id: {{ $folder->id }} })" data-toggle="tooltip" data-placement="bottom"
            title="{{ $folder->name }}">
            <div class="file_name">
    ...
    @if ($folder->subfolders_count)
        <div class="collapse pl-3" id="subfolder_{{ $folder->id }}">
            @include('folder.partials.file-list', [
                'folders' => $folder->subfolders()->withCount('subfolders')->get(),
            ])
        </div>
    @endif
@endforeach
