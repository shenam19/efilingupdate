<div class="card">
    <a href="{{ route('media.show',$media->uuid)}}" target="_blank">
        <img class="card-img-top " height="140" src="{{ getThumb(getExtension($media->file_name),$media->uuid)}}" alt="Dist Photo 1">
    </a>
    @if(isset($action))
    <div class="card-img-overlay d-flex flex-column justify-content-start p-0" style="height:15%;">
        <div class="px-3 py-2"  style="background:rgb(0,0,0,0.8)">
            ...
            @elseif($action ==='attachments')
                ...
                <button type="button" class="btn btn-link text-danger p-0 float-right" wire:click="removeMedia('{{$media->id}}')" ><i class="fas fa-times"></i></button>
        
            @elseif($action == 'media')
                @if($type == 'mine')
                    <div class="d-flex justify-content-between small">
                        <button wire:click="setEditMedia('{{$media->id}}')" type="button" class="btn text-light p-0" data-toggle="modal" data-target="#edit_media"><i class="fas fa-edit"></i></button>
                        <form action="{{ route('media.destroy',$media)}}" method="POST">
                            @csrf
                            @method('DELETE')

                            <button type="submit" class="btn text-danger p-0"><i class="fas fa-trash-alt"></i></button>
                        </form>
                    </div>
                @else
    ...
    <a href="{{ route('media.show',$media->uuid)}}" target="_blank">
        <div class="p-2 bg-light">
            <p class="text-dark mb-0" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">{{$media->name}}</p>
